...
interface Post {
  id: string; title: string; brand: string; image_url: string;
}
...
export function Home() {
  ...
  useEffect(() => {
    const fetchLatestPosts = async () => {
      ...
      } catch (error) { console.error("Erro ao buscar vitrine:", error); }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
              {latestPosts.map((post, index) => (
                <Link key={post.id} to={`/blog/${post.id}`} className="group block cursor-pointer glass-dark rounded-xl p-4 hover:border-white/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-glass hover-glitch bg-black/60">
                  <div className="relative overflow-hidden aspect-[4/5] bg-black mb-6 rounded-lg reveal-image border border-white/5">
                    <img src={post.image_url} alt={post.title} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 group-hover:scale-110 transition-all duration-[1.5s] ease-out filter grayscale group-hover:grayscale-0" />
                  ...
                  <div className="px-2">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-[9px] tracking-[0.3em] uppercase font-bold text-gray-400 group-hover:text-neon-red transition-colors">{post.brand}</span>
                    </div>
                    <h3 className="text-xl font-display leading-snug text-white group-hover:text-gray-300 transition-colors line-clamp-2">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
