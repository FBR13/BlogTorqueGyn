    ...
    <div className="bg-black min-h-screen flex flex-col selection:bg-neon-red selection:text-white relative z-0">
      ...
      <div className="pt-32 md:pt-40 pb-12 md:pb-16 px-6 border-b border-white/10 text-center relative overflow-hidden z-10">
        ...
        <p className="text-gray-400 mt-6 max-w-xl mx-auto text-sm md:text-base font-light tracking-wide">
          Análises profundas, novidades do setor e bastidores da engenharia automotiva de alta performance.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-32 md:py-40 text-center flex flex-col items-center z-10 relative">
          ...
          <p className="text-[10px] md:text-[11px] font-display tracking-[0.25em] uppercase text-gray-500">Nenhum editorial publicado.</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="relative overflow-hidden aspect-video md:aspect-[4/5] bg-black mb-6 rounded-lg border border-white/5 shrink-0">
                  ...
                  <div className="absolute top-4 left-4 glass-dark px-4 py-2 rounded-full flex items-center gap-2 bg-black/80 border border-white/10 shadow-xl">
                    ...
                    <span className="text-[9px] tracking-[0.2em] font-display font-bold uppercase text-neon-red">Ler Matéria</span>
                  </div>
                </div>
                ...
                <div className="px-2 flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className="w-8 h-[1px] bg-white/30 group-hover:w-12 group-hover:bg-neon-red transition-all duration-500" />
                      <span className="text-[9px] tracking-[0.3em] uppercase font-bold text-gray-500 group-hover:text-white transition-colors">{post.brand}</span>
                    </div>
                    <span className="text-[8px] tracking-[0.2em] uppercase text-gray-600">
                      {new Date(post.created_at).toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                  
                  <h3 className="text-xl font-display leading-snug text-white group-hover:text-neon-red transition-colors line-clamp-2 mb-3">
                    {post.title}
                  </h3>
                  ...
                  <div className="flex items-center gap-2 text-[10px] tracking-widest font-bold uppercase text-gray-500 group-hover:text-white transition-colors mt-auto">
                    Acessar Conteúdo <ArrowRight size={14} className="group-hover:translate-x-2 group-hover:text-neon-red transition-all" />
                  </div>
                </div>
              </Link>
            ))}
